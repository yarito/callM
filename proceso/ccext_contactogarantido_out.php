...
// log
$LOG = "/home/callcenter/interfaz/cc_externo/contactogarantido/log/out.log";
// sftp
$SFTP = 1;
...
$SQL = "CREATE TABLE EXPORT_CALLEXT_".$DATE." as 
        SELECT DISTINCT ca.IDCARTERA, ca.IDMOROSO
          FROM CARTERA ca
          JOIN SUCURSALES s ON s.SUCURSAL = ca.SUCURSAL
          JOIN USUARIOPERFILES up ON up.IDSUCURSAL = s.IDSUCURSAL
          JOIN USUARIOS u ON u.IDUSUARIO = up.IDUSUARIO 
         WHERE u.CODIGO LIKE '".$IDUSUARIO."'
           AND ca.TIPOPROCESO in ('1AT','002','003');";
$result = mysql_query($SQL, $conn);
savelog((($result) ? "OK" : "ERR")." TABLE: EXPORT_CALLEXT_".$DATE.(($result) ? "" : "\n".mysql_error($conn)));

$SQL = "create index a343 on EXPORT_CALLEXT_".$DATE." (IDCARTERA)";
$result = mysql_query($SQL, $conn);
savelog((($result) ? "OK" : "ERR")." create index a343".(($result) ? "" : "\n".mysql_error($conn)));

$SQL = "create index a3423 on EXPORT_CALLEXT_".$DATE." (IDMOROSO)";
$result = mysql_query($SQL, $conn);
savelog((($result) ? "OK" : "ERR")." create index a3423".(($result) ? "" : "\n".mysql_error($conn)));

$SQL = "create index a3a343 on EXPORT_CALLEXT_".$DATE." (IDMOROSO,IDCARTERA)";
$result = mysql_query($SQL, $conn);
savelog((($result) ? "OK" : "ERR")." create index a3a343".(($result) ? "" : "\n".mysql_error($conn)));
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/********** TABLA TEMPORAL **********/
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
$SQL = "SELECT LPAD(IFNULL(ca.IDCARTERA,''),11,' ') IDCARTERA,
          ...
          JOIN EXPORT_CALLEXT_".$DATE."  exp on ca.IDCARTERA=exp.IDCARTERA
...
$SQL = "SELECT LPAD(IFNULL(s.IDCARTERA,''),11,' ') IDCARTERA,
       ...
       WHERE s.ESTADO=1
         AND s.EXT != '".$EXT."'";
//AND DATE(s.fechaalta)>=DATE_SUB('".date('Y-m-d')."', INTERVAL 3 DAY)
$result = mysql_query($SQL, $conn);
...
while($rs = mysql_fetch_array($result)) {
  ...
  if ($cntRow==0) {
    // open file
    $fp = fopen($PATH."tmp/".$FILE['SEGUIMIENTO'].".".str_pad($fileName,5,"0", STR_PAD_LEFT), "w");
    savelog(((!$fp) ? 'ERR FILE: ':'OK FILE: ').$PATH."tmp/".$FILE['SEGUIMIENTO'].".".str_pad($fileName,5,"0", STR_PAD_LEFT));
  }
